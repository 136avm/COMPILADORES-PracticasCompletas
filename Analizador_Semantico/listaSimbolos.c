...
void imprimirLS(Lista lista) {
  PosicionLista p;
  int numeroCadena = 1;
  // Primero imprime todas las cadenas
  p = lista->cabecera->sig;
  while (p != NULL) {
    if (p->dato.tipo == STRING) {
      printf("$str%d:\n\t.asciiz %s\n", numeroCadena, p->dato.nombre);
      numeroCadena++;
    }
    p = p->sig;
  }

  // Luego imprime todas las variables y constantes
  p = lista->cabecera->sig;
  while (p != NULL) {
